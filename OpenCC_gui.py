...
class param():
    ...
    color_theme = 'blue'
    app_mode = 'dark'
    fonts = ('Microsoft JhengHei UI', 14)
...
class radBtn_frame(ctk.CTkFrame):
    def __init__(self, master, title, values, font, ini_state):
...
class openCCgui(ctk.CTk):
    def __init__(self):
        ...
        self.radio_srcf = radBtn_frame(self, title=param.radlist.src_title, values=param.radlist.src_lang, font=param.rad_fonts, ini_state='normal')
        self.radio_srcf.grid(row=0, column=0, padx=(10,10), pady=(10,10), sticky='nsew')
        self.radio_desf = radBtn_frame(self, title=param.radlist.dest_title, values=param.radlist.dest_lang, font=param.rad_fonts, ini_state='disabled')
        self.radio_desf.grid(row=0, column=1, padx=(10,10), pady=(10,10), sticky='nsew')
        
        self.selfile = ctk.CTkLabel(self, text='目標檔案:', font=param.fonts)
        self.selfile.grid(row=1, column=0, padx=(10,55), pady=(10,10), sticky='e')
        self.selbtn = ctk.CTkButton(self, text='選擇檔案', command=self.file_select, font=param.fonts)
        self.selbtn.grid(row=1, column=1, padx=(0,20), pady=(0,0), sticky='w')
        
        self.covbtn = ctk.CTkButton(self, text='開始轉換', command=self.submit, font=param.fonts)
        self.covbtn.grid(row=2, column=0, padx=(100,100), pady=(0,0), sticky='ew', columnspan=2)
    ...
    def submit(self):
        ...
        if(param.trans_json != None):
            try:
                converter(param.fpath, param.trans_json)
            except:
...
def converter(src_files, lang_json):       
